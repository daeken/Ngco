...
namespace Ngco {
	public struct Rect {
		...
		public Rect ClipTo(Rect region) {
			var tl   = new Point(Math.Max(TopLeft.X, region.TopLeft.X), Math.Max(TopLeft.Y, region.TopLeft.Y));
			var size = new Size(
				region.Size.Width == -1 ? Size.Width :
				Size.Width == -1 ? region.TopLeft.X + region.Size.Width - tl.X :
				Math.Min(region.TopLeft.X + region.Size.Width, TopLeft.X + Size.Width) - tl.X,
				region.Size.Height == -1 ? Size.Height :
				Size.Height == -1 ? region.TopLeft.Y + region.Size.Height - tl.Y :
		...
		public Rect Extend(Rect b) {
			var tl = new Point(Math.Min(TopLeft.X, b.TopLeft.X), Math.Min(TopLeft.Y, b.TopLeft.Y));
			var br = new Point(
