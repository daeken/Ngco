...
namespace Ngco {
	public class RICanvas {
		...
		public void DrawImage(SKBitmap bitmap, Point point, SKPaint paint = null) {
			var scaled = bitmap.Resize(new SKImageInfo(bitmap.Width * (int)Scale, bitmap.Height * (int)Scale), SKBitmapResizeMethod.Lanczos3);
			Canvas.DrawBitmap(scaled, point.X * Scale, point.Y * Scale, ScalePaint(paint));
		}
